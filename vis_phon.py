...
class LipReadingEvaluator:
    ...
    def calculate_phoneme_distance(self, phoneme1, phoneme2, use_weights=None):
        ...
        if use_weights and hasattr(self, 'feature_weights') and self.feature_weights:
            ...
            if total_weight > 0:
                ...
                raise ValueError(f"No weighted features found for phonemes '{phoneme1}' and '{phoneme2}'")
        else:
    ...
    def calculate_sequence_distance(self, seq1, seq2, use_weights=None):
        ...
        # Use the unified alignment function with direct reference to phoneme_distance
        return self.calculate_sequence_alignment(
            seq1, 
            seq2, 
            cost_function=lambda item1, item2: self.calculate_phoneme_distance(item1, item2, use_weights=use_weights)
        )
    ...
    def map_phoneme_to_viseme(self, phoneme, default_value=None):
        ...
        Raises an error if the phoneme isn't recognized.
        
        Args:
            phoneme: The phoneme to map
            default_value: DEPRECATED - kept for backward compatibility
                
        Returns:
            int: The viseme ID (0-21) that corresponds to this phoneme
            
        Raises:
            ValueError: If the phoneme isn't found in the mapping
        """
        ...
        # Raise error if phoneme isn't recognized
        raise ValueError(f"Unrecognized phoneme: '{phoneme}'. Add it to phoneme_to_viseme mapping.")
...
class WeightedLipReadingEvaluator(LipReadingEvaluator):
    ...
    def calculate_phonetic_feature_weights(self):
        ...
        for feature_idx, feature in enumerate(all_features):
            ...
            total_viseme_classes = len(self.viseme_id_to_name)  # Get from actual viseme class count instead of hardcoding
