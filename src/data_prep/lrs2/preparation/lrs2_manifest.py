...
#                     fo.write('\t'.join([
#                         fid,
#                         os.path.abspath(f"{args.lrs2}/{fid}.mp4"),
#                         os.path.abspath(f"{args.lrs2}/{fid}.wav"),
#                         str(nf_video),
#                         str(nf_audio)
#                     ])+'\n')
...
#     # Read split files
#     train_ids = set(line.strip().split()[0] for line in open(f"{args.lrs2}/train.txt"))
#     pretrain_ids = set(line.strip().split()[0] for line in open(f"{args.lrs2}/pretrain.txt"))
...
#         # Handle pretrain folder entries
#         if fid.startswith('pretrain/'):
#             base_id = '/'.join(fid.split('/')[1:])  # Remove pretrain/ prefix
#             base_id = base_id.replace('_00', '')    # Remove _00 suffix if present
#             if base_id in pretrain_ids:
#                 train.append(data_item)
#         # Handle main folder entries
#         elif fid.startswith('main/'):
#             base_id = '/'.join(fid.split('/')[1:])  # Remove main/ prefix
#             if base_id in train_ids:
#                 train.append(data_item)
#             elif base_id in valid_ids:
#                 valid.append(data_item)
#             elif base_id in test_ids:
#                 test.append(data_item)

#     output_dir = f"{args.lrs2}/data"
...
#                 fo.write('/\n')
#                 for fid, _, nf_audio, nf_video in data:
#                     clean_fid = fid.replace('main/', '')
#                     fo.write('\t'.join([
#                         clean_fid,
#                         os.path.abspath(f"{args.lrs2}/{clean_fid}.mp4"),
#                         os.path.abspath(f"{args.lrs2}/{clean_fid}.wav"),
#                         str(nf_video),
#                         str(nf_audio)
#                     ])+'\n')
#             with open(f"{target_dir}/{name}.wrd", 'w') as fo:
...
#     output_dir = f"{args.lrs2}/processed_data"
#     print("Setting up processed data directory")
#     setup_target(output_dir, train, valid, test)
