...
if dataset == "lrs3":
    ...
    if args.subset in ["val", "test"]:
        subset_file_path = os.path.join(args.data_dir, args.subset + ".txt")
        print(f"Using subset file path: {subset_file_path}")
        filenames = [
            os.path.join(args.data_dir, "main", _.split()[0] + ".mp4")
            for _ in open(subset_file_path).read().splitlines()
        ]
    elif args.subset == "train":
        train_file_path = os.path.join(args.data_dir, "train.txt")
        pretrain_file_path = os.path.join(args.data_dir, "pretrain.txt")
        print(f"Using train file path: {train_file_path}")
        print(f"Using pretrain file path: {pretrain_file_path}")
        
        filenames = [
            os.path.join(args.data_dir, "main", _.split()[0] + ".mp4")
            for _ in open(train_file_path).read().splitlines()
        ]
        pretrain_filenames = [
            os.path.join(args.data_dir, "pretrain", _.split()[0] + ".mp4")
            for _ in open(pretrain_file_path).read().splitlines()
        ]
...
# Example Usage: python preprocess_lrs2_in_lrs3_style.py --data-dir /data/ssd2/data_rishabh/lrs2 --landmarks-dir /data/ssd2/data_rishabh/lrs2/LRS2_landmarks --detector mediapipe --root-dir /data/ssd2/data_rishabh/lrs2/training --dataset lrs2 --subset val