# Weighted Analysis with Phoneme and Viseme Cost functions for IPA and G2P
#!/usr/bin/env python3
import os
import json
import numpy as np
...
class LipReadingEvaluator:
    ...
    def __init__(self, use_ipa=False):
        ...
        # Define G2P phoneme-to-viseme mapping (CMU Dictionary style)
        self.g2p_phoneme_to_viseme = {
            # 0: Silence (only empty string and space needed now that punctuation is handled separately)
            '': 0, ' ': 0,
            
            # 1: Open/central vowels
            'AE': 1, 'AH': 1, 'AE0': 1, 'AE1': 1, 'AE2': 1, 'AH0': 1, 'AH1': 1, 'AH2': 1,
            'AX': 1, 'AX0': 1, 'AX1': 1, 'AX2': 1,  # Add schwa variant (used in some CMU extensions)
            
            # 2: Open back vowels
            'AA': 2, 'AA0': 2, 'AA1': 2, 'AA2': 2,
            
            # 3: Open-mid back rounded
            'AO': 3, 'AO0': 3, 'AO1': 3, 'AO2': 3,
            ...
            # 5: R-colored vowels
            'ER': 5, 'ER0': 5, 'ER1': 5, 'ER2': 5,
            'AXR': 5, 'AXR0': 5, 'AXR1': 5, 'AXR2': 5,  # Add r-colored schwa variant
            
            # 6: Close front vowels + /j/
            'IY': 6, 'IH': 6, 'Y': 6, 
            'IY0': 6, 'IY1': 6, 'IY2': 6, 
            'IH0': 6, 'IH1': 6, 'IH2': 6,
            'EY': 6, 'EY0': 6, 'EY1': 6, 'EY2': 6,
            
            # 7: Close back rounded + /w/
            'UW': 7, 'W': 7, 'WH': 7,  # Add voiceless labiovelar fricative (as in "which")
            'UW0': 7, 'UW1': 7, 'UW2': 7,
            
            # 8: Close-mid back rounded
            'OW': 8, 'OW0': 8, 'OW1': 8, 'OW2': 8,
            
            # 9-11: Major diphthongs
            'AW': 9, 'AW0': 9, 'AW1': 9, 'AW2': 9,
            'OY': 10, 'OY0': 10, 'OY1': 10, 'OY2': 10,
            'AY': 11, 'AY0': 11, 'AY1': 11, 'AY2': 11,
            
            # 12: Glottal
            'HH': 12, 'HH0': 12, 'HH1': 12, 'HH2': 12,
            'Q': 12,  # Add glottal stop (sometimes used in CMU)
            
            # 13: Rhotic approximant
            'R': 13, 'R0': 13, 'R1': 13, 'R2': 13,
            'RR': 13,  # Trilled R variant (sometimes used)
            
            # 14: Lateral approximant
            'L': 14, 'L0': 14, 'L1': 14, 'L2': 14,
            'EL': 14,  # Syllabic L (as in "bottle")
            
            # 15: Alveolar fricatives
            'S': 15, 'Z': 15, 'S0': 15, 'S1': 15, 'S2': 15, 'Z0': 15, 'Z1': 15, 'Z2': 15,
            
            # 16: Post-alveolar sounds
            'SH': 16, 'ZH': 16, 'CH': 16, 'JH': 16,
            'SH0': 16, 'SH1': 16, 'SH2': 16,
            'ZH0': 16, 'ZH1': 16, 'ZH2': 16,
            'CH0': 16, 'CH1': 16, 'CH2': 16,
            'JH0': 16, 'JH1': 16, 'JH2': 16,
            'TS': 16, 'DZ': 16,  # Add affricates (as in "cats", "beds")
            
            # 17: Voiced dental fricative
            'DH': 17, 'DH0': 17, 'DH1': 17, 'DH2': 17,
            
            # 18: Labiodental fricatives
            'F': 18, 'V': 18, 'F0': 18, 'F1': 18, 'F2': 18, 'V0': 18, 'V1': 18, 'V2': 18,
            
            # 19: Alveolar stops, nasal + voiceless dental
            'T': 19, 'D': 19, 'N': 19, 'TH': 19,
            'T0': 19, 'T1': 19, 'T2': 19,
            'D0': 19, 'D1': 19, 'D2': 19,
            'N0': 19, 'N1': 19, 'N2': 19,
            'TH0': 19, 'TH1': 19, 'TH2': 19,
            'DX': 19, 'NX': 19,  # Add flap (as in "butter") and nasal flap
            'EN': 19,  # Syllabic N (as in "button")
            
            # 20: Velar consonants
            'K': 20, 'G': 20, 'NG': 20,
            'K0': 20, 'K1': 20, 'K2': 20,
            'G0': 20, 'G1': 20, 'G2': 20,
            'NG0': 20, 'NG1': 20, 'NG2': 20,
            'ENG': 20,  # Syllabic NG (rare, but possible)
            ...
            'B0': 21, 'B1': 21, 'B2': 21,
            'M0': 21, 'M1': 21, 'M2': 21,
            'EM': 21,  # Syllabic M (as in "rhythm")
        }
        
        # Define IPA phoneme-to-viseme mapping
        self.ipa_phoneme_to_viseme = {
            # 0: Silence (only empty string and space needed now that punctuation is handled separately)
            '': 0, ' ': 0,
        ...
        # Set the phoneme_to_viseme mapping based on the use_ipa flag
        self.phoneme_to_viseme = self.ipa_phoneme_to_viseme if use_ipa else self.g2p_phoneme_to_viseme
    ...
    def calculate_phoneme_distance(self, phoneme1, phoneme2, use_weights=None):
        ...
        try:
            ...
            if use_weights and hasattr(self, 'feature_weights') and self.feature_weights:
                ...
                if not self.use_ipa:
                    ipa_phoneme1 = self._convert_g2p_to_ipa_for_panphon(phoneme1)
                    ipa_phoneme2 = self._convert_g2p_to_ipa_for_panphon(phoneme2)
                    
                    # Skip if conversion failed
                    if not ipa_phoneme1 or not ipa_phoneme2:
                        print(f"Warning: Failed to convert G2P phonemes '{phoneme1}' and/or '{phoneme2}' to IPA")
                        raise ValueError(f"Failed to convert G2P phonemes '{phoneme1}' and/or '{phoneme2}' to IPA")
                        
                    # Get raw feature vectors from panphon using IPA
                    fv1 = self.ft.word_to_vector_list(ipa_phoneme1, numeric=True)
                    fv2 = self.ft.word_to_vector_list(ipa_phoneme2, numeric=True)
                else:
    ...
    def text_to_phonemes(self, text):
        ...
        # Process based on conversion approach (IPA or G2P)
        if self.use_ipa:
                ...
                while i < len(word):
                    ...
                    for phoneme_len in [3, 2, 1]:
                        if i + phoneme_len <= len(word):
                            ...
                            if candidate in self.ipa_phoneme_to_viseme:
            ...
            # G2P conversion gives us segmented phonemes
            raw_phonemes = self.g2p(normalized_text)
            
            # Process the phoneme sequence
            i = 0
            while i < len(raw_phonemes):
                # Handle spaces
                if raw_phonemes[i].strip() == '':
                    processed_phonemes.append(' ')
                    i += 1
                    continue
                
                # Check for combined phonemes (affricates, diphthongs)
                if i < len(raw_phonemes) - 1:
                    combined = raw_phonemes[i] + raw_phonemes[i+1]
                    if combined in self.g2p_phoneme_to_viseme:
                        processed_phonemes.append(combined)
                        i += 2
                        continue
                
                # Handle single phoneme
                processed_phonemes.append(raw_phonemes[i])
                i += 1
    ...
    def map_phoneme_to_viseme(self, phoneme, default_value=-1):
        ...
        """
        try:
            # Choose the appropriate phoneme-to-viseme mapping based on use_ipa flag
            phoneme_mapping = self.ipa_phoneme_to_viseme if self.use_ipa else self.g2p_phoneme_to_viseme
...
class WeightedLipReadingEvaluator(LipReadingEvaluator):
    ...
    def _convert_g2p_to_ipa_for_panphon(self, phoneme):
        """
        Convert G2P phoneme to IPA format for compatibility with panphon
        
        Args:
            phoneme: G2P phoneme to convert
            
        Returns:
            str: IPA equivalent or empty string if no mapping
        """
        # Basic mapping of CMU (G2P) phonemes to IPA - no stress markers
        cmu_to_ipa = {
            ...
            'W': 'w', 'Y': 'j', 'Z': 'z', 'ZH': 'ʒ',
            # Add mappings for stressed variants without the stress removal step
            'AA0': 'ɑ', 'AA1': 'ɑ', 'AA2': 'ɑ',
            'AE0': 'æ', 'AE1': 'æ', 'AE2': 'æ',
            'AH0': 'ʌ', 'AH1': 'ʌ', 'AH2': 'ʌ',
            'AO0': 'ɔ', 'AO1': 'ɔ', 'AO2': 'ɔ',
    ...
    def calculate_phonetic_feature_weights(self):
        ...
        """
        print("Calculating phonetic feature weights...")
        weights = {}
        
        # Generate phoneme inventory directly from the complete mapping
        # Use the appropriate phoneme-to-viseme mapping based on mode
        ...
        print(f"Using {len(phoneme_inventory)} phonemes from {'IPA' if self.use_ipa else 'G2P'} inventory for weight calculation")
        
        # Get panphon feature names
        all_features = self.ft.names
        print(f"Analyzing {len(all_features)} panphon phonetic features")
        
        # Create sample IPA phonemes for feature extraction if using G2P
        ipa_phonemes = {}
        if not self.use_ipa:
            print("Converting G2P phonemes to IPA for feature extraction")
            for phoneme in phoneme_inventory:
                ipa = self._convert_g2p_to_ipa_for_panphon(phoneme)
                if ipa:
                    ipa_phonemes[phoneme] = ipa
            print(f"Converted {len(ipa_phonemes)} G2P phonemes to IPA representation")
        ...
        for feature_idx, feature in enumerate(all_features):
            ...
            for phoneme in phoneme_inventory:
                ...
                try:
                    ...
                    if not self.use_ipa and phoneme in ipa_phonemes:
                        # Use IPA conversion for G2P mode
                        ipa_phoneme = ipa_phonemes[phoneme]
                        fv = self.ft.word_to_vector_list(ipa_phoneme, numeric=True)
                    else:
                        # Use original phoneme for IPA mode
...
def main():
    ...
    if args.weights:
        ...
        if args.ipa:
            ...
            weights_path = os.path.join(args.save_dir, 'viseme_weights_g2p.json')
    ...
    if args.compare:
        ...
        print("\n=== Comparing Standard vs. Phonetically-Weighted Viseme Scores ===\n")
